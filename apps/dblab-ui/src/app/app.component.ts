...
const BACK_URL = 'http://localhost:3333/api';
...
export class AppComponent implements OnInit{
  ...
  getTablesData() {
    for(const tab of this.tabs) {
      this.http$.get<CustomerDto[] | DeliveryDto[] | DealDto[] | ProductDto[]>(`${BACK_URL}/getallrows`, {
        params: {
  ...
  openAddRowDialog() {
    ...
    dialogRef.afterClosed().subscribe(row => {
      ...
      this.http$.post(`${BACK_URL}/addRow`, {
      ...
      .subscribe({
        next: (row) => {
          ...
          (row as DealDto).product_id = (row as DealDto).product?.id;
          (row as DealDto).delivery_id = (row as DealDto).delivery?.id;
          currentDataSet.push(row as CustomerDto & DealDto & DeliveryDto & ProductDto);
          const currentTableView = this.tableViews.toArray()[this.currentTabIndex];
          currentTableView.table.renderRows();
  ...
  deleteRow(row: DataDto) {
    const currentTab = this.tabs[this.currentTabIndex];
    this.http$.post(`${BACK_URL}/deleteRow`, {
      tableName: currentTab.tableName,
      row: row
    })
    .subscribe({
      next: () => {
        ...
        currentTab.dataSet = (currentDataSet as any).filter(
                          // eslint-disable-next-line @typescript-eslint/no-explicit-any
                          (el: any) => el[currentTab.primaryKeyColName]
                          // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  updateRow(row: DataDto) {
    ...
    this.http$.post(`${BACK_URL}/updateRow`, {
      tableName: currentTab.tableName,
      row: row
    })
    .subscribe({
  ...
  updateDealRelations(row: DealDto) {
    row.customer = (this.tabs[0].dataSet as CustomerDto[])
                    .filter(customerRow => customerRow.id == row.customer_id)[0];
    row.delivery = (this.tabs[2].dataSet as DeliveryDto[])
                    .filter(deliveryRow => deliveryRow.id == row.delivery_id)[0];
    row.product = (this.tabs[3].dataSet as ProductDto[])
                    .filter(productRow => productRow.id == row.product_id)[0];
  ...
  truncAllTables() {
    this.http$.post(`${BACK_URL}/truncAllTables`, {})
    .subscribe({
      complete: () => {
        for(const tab of this.tabs) {
          tab.dataSet = [];
  ...
  dropDb() {
    this.http$.post(`${BACK_URL}/dropDb`, {})
    .subscribe({
      complete: () => {
        for(const tab of this.tabs) {
          tab.dataSet = [];
  ...
  createDb() {
    this.http$.post(`${BACK_URL}/createDb`, {})
    .subscribe({
      complete: () => {
  ...
  fillAllTables() {
    this.http$.post(`${BACK_URL}/fillTables`, {})
    .subscribe({
      complete: () => {
  ...
  truncCurrentTable() {
    const currentTab = this.tabs[this.currentTabIndex];
    this.http$.post(`${BACK_URL}/truncTable`, {
