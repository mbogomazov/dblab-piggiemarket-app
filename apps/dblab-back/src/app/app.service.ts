...
export class AppService {
  ...
  async truncAllTables() {
    return await getConnection()
    .query(`TRUNCATE TABLE deal CASCADE; TRUNCATE TABLE customer CASCADE; TRUNCATE TABLE delivery CASCADE; TRUNCATE TABLE product CASCADE;`);
  }
  ...
  fillTables() {
    let jsonCustomer = csvToJson.fieldDelimiter(',').getJsonFromCsv('/home/noname/projects/postgres-project/dblab/apps/dblab-back/src/data/customer.csv');
    let jsonDelivery = csvToJson.fieldDelimiter(',').getJsonFromCsv('/home/noname/projects/postgres-project/dblab/apps/dblab-back/src/data/delivery.csv');
    let jsonProduct = csvToJson.fieldDelimiter(',').getJsonFromCsv('/home/noname/projects/postgres-project/dblab/apps/dblab-back/src/data/product.csv');


    let jsonDeal = csvToJson.fieldDelimiter(',').getJsonFromCsv('/home/noname/projects/postgres-project/dblab/apps/dblab-back/src/data/deal.csv');
    jsonDeal = jsonDeal.map(row => {
      ...
      jsonCustomer.filter(item => item.id === customer_id)[0]['deal_number'] = row['deal_number'];
      jsonProduct.filter(item => item.id === product_id)[0]['deal_number'] = row['deal_number']
      jsonDelivery.filter(item => item.id === delivery_id)[0]['deal_number'] = row['deal_number']
      row['customer'] = jsonCustomer.filter(item => item.id === customer_id)[0];
      row['product'] = jsonProduct.filter(item => item.id === product_id)[0];
      row['delivery'] = jsonDelivery.filter(item => item.id === delivery_id)[0];
